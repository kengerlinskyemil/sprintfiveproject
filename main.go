...
// Формула расчета:
// количество_повторов * длина_шага / м-в-км.
...
// Формула: преодолённая_дистанция_за_тренировку_в_км / время_тренировки_в_часах
...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	return ((float64(CaloriesMeanSpeedMultiplier)*r.meanSpeed() + CaloriesMeanSpeedShift) * r.Weight / float64(MInKm) * r.Duration.Hours() * float64(MinInHours))
}
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	return ((CaloriesWeightMultiplier*w.Weight + (math.Pow(w.meanSpeed(), 2)/w.Height)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * float64(MinInHours))
}
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	return (s.meanSpeed() + float64(SwimmingCaloriesMeanSpeedShift)) * float64(SwimmingCaloriesWeightMultiplier) * s.Weight * float64(s.Duration.Hours())
}
